(function(root){
  ...
  function build(r,input,{figure=null}={}){
    ...
    const legal=root.NCYSC01MemberAnalysis.legalReferences[d.design.legalBasis];
    const inputRows=[['ข้อกำหนดอ้างอิงงาน',legal.label+' · คำนวณตามโปรไฟล์และโหลดที่ระบุ ไม่ใช่ผลรับรองตามกฎหมาย'],['จุดรองรับ',{'concrete-wall':'ผนังคอนกรีต','concrete-beam':'คานคอนกรีต','dual-columns':'เสาซ้าย–ขวาที่ฐาน + คานเชื่อม',hbeam:'เพลทเชื่อมกับ H-beam'}[d.support.type]+' · ยึดแน่น'],
...
const F=root.NCYSC01MemberReportFigures,math=[];
    ...
    const anchorEquation=q=>{
      const x=q.detail||q,formulas={
        breakoutT:['Ncb = (ANc/ANco) ψed ψec,x ψec,y ψcp kc λ √f′c hef^1.5',`ANc ${n(x.ANc)} / ANco ${n(x.ANco)} mm²; hef,calc ${n(x.hef)} mm; ψed ${n(x.psiEd)}; ψec ${n(x.psiEc)}; ψcp ${n(x.psiCp)}; Nb ${n(x.Nb)} kN`],
        bond:['Nag = (ANa/ANao) ψed ψec,x ψec,y ψcp Nba; Nba = τ π d hef',`ANa ${n(x.ANa)} / ANao ${n(x.ANa0)} mm²; cNa ${n(x.cNa)} mm; τ ${n(x.tau)} MPa; ψed ${n(x.psiEd)}; ψec ${n(x.psiEc)}; ψcp ${n(x.psiCp)}; Nba ${n(x.Nba)} kN`],
        breakoutV:['Vcb = (AVc/AVco) ψec ψed ψc ψh Vb; DC = Vperp/φVcb + Vparallel/φVparallel',`ขอบ ${x.direction}; AVc ${n(x.AVc)} / AVco ${n(x.AVco)} mm²; ลึกหน้าขอบ ${n(x.depth)} mm; ca1 ${n(x.c1)} mm; ψec/ψed/ψc/ψh ${[x.psiEc,x.psiEd,x.psiC,x.psiH].map(v=>n(v)).join(' / ')}; Vb ${n(x.Vb)} kN`],
        pryout:['φVcp = φV kcp min(Ncb, Nag)',`φV ${r.connections.profile.phiConcreteV}; kcp ${x.kcp}; Ncb ${n(x.Ncb)} kN; Nag ${n(x.Nag)} kN`],
        concreteInteraction:['nr ≤ 1; vr ≤ 1; if nr > 0.2 and vr > 0.2: nr + vr ≤ 1.2',`nr ${n(x.nr)}; vr ${n(x.vr)}; ใช้ค่าควบคุมเหล็ก / คอนกรีต / แรงยึดเหนี่ยวในกรณีเดียวกัน`],
        'bond-sustained':['Nua,sustained ≤ 0.55 φbond Nba',`Nua,sustained ${n(x.demand)} kN; φbond ${d.connection.product.phiBond}; Nba ${n(x.Nba)} kN`]
      },f=formulas[q.id];
      return f?eq(q.label,f[0],f[1]+(Number.isFinite(x.design)?`; กำลังออกแบบ ${n(x.design)} kN`:''),`D/C ${n(q.ratio)} ≤ 1 — OK`,q.plateId+' / '+q.caseName,x.methodSource||'ACI 318-19'):
        `<h2>${esc(q.label)}</h2><p>${esc(q.plateId||'')} · ${esc(q.caseName||'')}</p><p>${esc(q.note||'')}</p>${table(['รายการ','ผล'],[['ใช้กำลัง / เกณฑ์',`${n(q.ratio)} ≤ 1 — OK`]])}`;
    };
    ...
    const hw=root.NCYSC01MemberAnalysis.hardware(d),a=V.rootActions(d,r),connections=r.connections,view=F.connection(d,r),rootCase=connections.all.find(q=>q.caseName===view.caseName&&q.plateId===view.plateId);
    ...
    add('เพลทและรอยเชื่อม',`${pair(panel('PL1 · ผังเพลท / หน้าตัดเหล็ก',view.front,view.plateId+' · '+view.caseName),panel('SECTION A–A · จุดรองรับ',view.section,connections.hbeam?'เพลทเชื่อมกับ H-beam':'รูปตัดพุก / ระยะฝัง'))}
      ...
      ${table(['รายละเอียดที่เลือก','ค่า'],[['เพลท',`${rootCase.plate.width} × ${rootCase.plate.height} × ${d.plate.thicknessMM} mm`],['ขนาดเริ่มต้น',d.connection.plateSizing==='square-inch'?`ด้านใหญ่สุดของเหล็ก + 2 นิ้ว → เริ่ม ${n(root.NCYSC01MemberAnalysis.plateMinimum(d)/25.4)} นิ้ว; ขนาดใช้งานตามแบบและผลตรวจ`:'รายละเอียดขนาดของงานเดิม'],['เกรดเพลท',`Fy ${d.connection.plateFy} MPa / Fu ${d.connection.plateFu} MPa`],['W1 · เหล็ก → เพลท',`${d.connection.weldPattern} · ขา ${d.connection.weldSizeMM} mm · Fexx ${d.connection.weldFexx} MPa · ตัวคูณแนวเชื่อม ${d.connection.weldLengthFactor}`],...(!connections.hbeam&&hw.id==='iso7089'?[['รูและแหวน',`${rootCase.group.forces.length} รูต่อเพลท · รูเพลท Ø${d.plate.anchorMM+2}; แหวน ISO 7089 OD ${hw.outsideMM} × ${hw.thicknessMM} mm · เกรด/ผิวให้ตรงแกนพุก`]]:[]),...(connections.hbeam?[['W2 · เพลท → H-beam',`รอบเพลท ขา ${d.connection.supportWeldSizeMM} mm; หน้ารองรับแข็งเต็มแนว`]]:[])])}
      ...
      ${connections.checks.filter(q=>['plate','weld','support-weld','hardware-fit','support-fit'].includes(q.id)).map(q=>{
        ...
        if(q.id.includes('weld')&&detail.loadPath?.method==='outside-hss'){
          const l=detail.loadPath,g=detail.geometry;
          return eq(q.label,'Aw = 0.707 w Leff; σeq = resultant(N/Aw, Mx y/Ix, My x/Iy, V/Aw, T r/J)',`w ${g.leg} mm; Leff ${n(g.effectiveLength)} mm; σeq ${n(detail.maxStress)} MPa; φFn ${n(detail.cap)} MPa`,`กำลังแนวเชื่อม D/C ${n(detail.strengthRatio)}; เนื้อเหล็ก ${n(detail.baseRatio)} — OK`,q.caseName,'AISC J2 / J4')
            +eq('แนวเชื่อมรอบและบริเวณมุม HSS','qwall = Fy,HSS tdesign; φRw/mm = φw (0.6 Fexx) (0.707 w) f β; DC = max(qwall/φRw, DCเฉพาะที่)',`qwall ${n(l.wallYieldPerMM)} N/mm; φRw ${n(l.weldCapacityPerMM)} N/mm; ตรวจพัฒนาแรงคราก ${n(l.developmentRatio)}`,`D/C ควบคุมรอยเชื่อม ${n(q.ratio)} ≤ 1 — OK`,q.caseName,'DG1 4.3.2 / AISC J2 / J4')
            +table(['พุก / แนวเชื่อมเฉพาะที่','Leff (mm)','qn / qv (N/mm)','D/C'],l.local.map(x=>[x.id,n(x.length),n(x.qn)+' / '+n(x.qv),n(x.ratio)]))+'<p>'+esc(q.note)+'</p>';
        }
        if(q.id.includes('weld')){const g=detail.geometry||q.geometry,l=detail.loadPath;return eq(q.label,'Aw = 0.707 w Leff; σeq = resultant(N/Aw, Mx y/Ix, My x/Iy, V/Aw, T r/J); เนื้อเหล็ก: |σ|/φFn + τ/φFv',`w ${g?.leg??d.connection.weldSizeMM} mm; Leff ${n(g?.effectiveLength)} mm; σeq ${n(detail.maxStress)} MPa; φRn ${n(detail.cap)} MPa; เนื้อเหล็ก φFn ${n(detail.baseCap)} / φFv ${n(detail.baseShearCap)} MPa`,`σeq/φRn = ${n(detail.strengthRatio??q.ratio)}; เนื้อเหล็ก ${n(detail.baseRatio)}; ขา ${n(g?.leg)} mm อยู่ในช่วง ${n(q.minSize??detail.minSize)}–${n(q.maxSize??detail.maxSize)} mm — OK`,q.caseName,q.label)+(l?eq('รอยเชื่อมตามเส้นทางแรง DG1','Fface = |N/2 ± M/(H − t)|; qn = Fface/Leff; qv = |Vy|/(2 Leff); σeq = √(qn²+qv²)/(0.707w)',`Fface ${l.faceForces.map(v=>n(v/1000)).join(' / ')} kN; Leff ${n(l.length)} mm/ด้าน; σeq ${n(l.stress)} MPa; φFn ${n(l.cap)} MPa`,`เส้นทางแรง D/C ${n(l.ratio)}; ควบคุมรอยเชื่อม D/C ${n(q.ratio)} ≤ 1 — OK`,q.caseName,'DG1 / รอยเชื่อมด้านรับแรง'):'')+'<p>'+esc(q.note)+'</p>';}
        ...
        if(q.id==='plate'&&['dg1-2024-aci318-19-r44','dg1-sti2024-aci318-19-r47'].includes(rootCase.method))return eq(q.label,'DG1: ΣT − C = N; ΣTy − Cy = Mx; C = fp B Y; t_req = √[6 max(mt,mc)/(φFy)]',`${q.note}; mt ${n(detail.tensionMoment)} / mc ${n(detail.bearingMoment+detail.lateralMoment)} N·mm/mm; φFy ${n(r.connections.profile.phiPlateY*d.connection.plateFy)} MPa; t ${d.plate.thicknessMM} mm`,`ดัดดึง ${n(detail.tensionRatio)}; ดัดแรงกดสองทิศ ${n(detail.compressionRatio)}; รวมเฉือน/รู/หน้าตัด ${n(q.ratio)} ≤ 1 — OK`,q.caseName,'AISC DG1 2024 / เพลท')+table(['พุก / แถบเพลท','แรง T (kN)','แขน a (mm)','กว้าง be (mm)'],detail.strips.flatMap(a=>(a.modes||[a]).map(m=>[a.id+(m.mode?' / '+m.mode:''),n(a.T),n(m.lever),n(m.be)])))+table(['ตรวจเพลทเพิ่มเติม','D/C'],[['รู / แรงแบก',n(detail.holeRatio)],['เฉือนหน้าตัดสุทธิ',n(detail.netRatio)],['ฉีกขอบ / block shear',n(detail.blockRatio)],['เฉือนทะลุรอบรู · φR = '+n(detail.punchCap)+' kN',n(detail.punchRatio)]])+'<p>'+esc(detail.assumption)+'</p>';
    ...
    if(!connections.hbeam){
      ...
      ${d.connection.productSelection.id===root.NCYSC01DesignProducts.ID?`<p>ฐานข้อมูล ${esc(d.connection.productSelection.version)} · <a href="${root.NCYSC01DesignProducts.SOURCE}">ESR-3814 May 2025</a> ตาราง14/15/19 · เจาะกระแทก · ${esc(d.connection.productSelection.moisture)} · อุณหภูมิ ${esc(d.connection.productSelection.temperature)} · เกรด ${esc(d.connection.productSelection.grade)}</p>${table(['ค่าผลิตภัณฑ์ที่ใช้','ค่า'],[['φNsa / φVsa (kN)',`${n(d.connection.product.NsaDesign)} / ${n(d.connection.product.VsaDesign)}`],['τcr / τuncr (MPa)',`${d.connection.product.tauCr} / ${d.connection.product.tauUncr}`],['φbond / cac (mm)',`${d.connection.product.phiBond} / ${n(d.connection.product.cac)}`],['ขอบขั้นต่ำ / ระยะขั้นต่ำ / คอนกรีตหนาขั้นต่ำ (mm)',`${d.connection.product.minEdge} / ${d.connection.product.minSpacing} / ${d.connection.product.minMemberThickness}`]])}`:''}
      ${table(['ผังรูพุก','ค่าจากตำแหน่งที่คำนวณ'],[['รูปแบบ',d.connection.anchorLayout==='outside-hss'?'พ้นด้านข้างหน้าตัดเหล็ก':'ผังเดิม'],['ระยะศูนย์รู sx / sy (mm)',n(Math.max(...view.forces.map(a=>a.x))-Math.min(...view.forces.map(a=>a.x)))+' / '+n(Math.max(...view.forces.map(a=>a.y))-Math.min(...view.forces.map(a=>a.y)))],['ศูนย์รูจากผิวเหล็กด้านข้าง (mm)',n(Math.min(...view.forces.map(a=>Math.abs(a.x)))-d.steel.B/2)]])}${table(['พุก','x (mm)','y (mm)','T (kN)','Vx (kN)','Vy (kN)'],view.forces.map(x=>[x.id,n(x.x),n(x.y),x.T,x.Vx,x.Vy]))}
      ${connections.checks.filter(q=>!['plate','weld','support-weld','hardware-fit','support-fit','geometry','solver','productGate'].includes(q.id)).map(anchorEquation).join('')}<h2>ระยะติดตั้ง / สมดุลแรง / ข้อมูลผลิตภัณฑ์</h2>${table(['รายการตรวจ','หลักฐาน / ผล'],connections.checks.filter(q=>['geometry','solver','productGate'].includes(q.id)).map(q=>[q.label,q.note+' · ✓ OK']))}`);
    }
    ...
    add('สรุปเลือกใช้ / แหล่งอ้างอิง',`<p class="sm-chosen">✓ เลือกใช้${tr?'คอร์ด':'คาน'}เหล็ก ${size(d.steel)} — OK${tr?'<br>✓ Web '+size(d.web)+' — OK':''}<br>✓ เลือกใช้แปเหล็ก ${size(d.purlin)} — OK<br>✓ เพลท ${n(a.plates[0].width)} × ${n(a.plates[0].height)} × ${d.plate.thicknessMM} mm — OK<br>✓ รอยเชื่อม ${d.connection.weldPattern} ขา ${d.connection.weldSizeMM} mm — OK${a.hbeam?'<br>✓ แนวเชื่อมเพลท → H-beam — OK':'<br>✓ พุก M'+d.plate.anchorMM+' ตามเอกสารที่ระบุ — OK'}${d.cable.enabled?'<br>✓ สลิงตาม EA / กำลังใช้งานที่ระบุ — OK':''}</p>
      ...
      <p class="sm-dl-reference">${d.support.type!=='hbeam'?'<a href="https://www.hilti.com/engineering/articles/aisc-design-guide-1">AISC DG1 (2024) — สมดุลแรงและเพลท</a> · <a href="https://www.concrete.org/store/productdetail.aspx?ItemID=318U19">ACI 318-19 Chapter 17 — พุกคอนกรีต</a> · ':''}<a href="${esc(root.NCYSC01SteelDesign.source)}">AISC 360-22 — E3/E7, F7, G4, H1, J2 และ J4</a> · <a href="https://www.aisc.org/aisc/solutions-center/engineering-faqs/8-welding/">AISC: ข้อจำกัดขารอยเชื่อมตามชนิดจุดต่อ</a> · <a href="https://people.duke.edu/~hpgavin/cee421/beam-element.pdf">Duke: Beam element</a>${tr?' · <a href="https://people.duke.edu/~hpgavin/cee421/truss-finite-def.pdf">Duke: Truss finite deformation</a>':''}</p>${d.connection.anchorLayout==='outside-hss'&&d.support.type!=='hbeam'?'<p><a href="'+root.NCYSC01CornerAnchors.SOURCE+'">STI 2024 Figure 4: แนวตรวจดัดเพลทตรงและมุม 45°</a></p>':''}<p>สมการและบรรทัดแทนค่าใช้ SI; ค่าประกอบแสดงตามหน่วยโปรไฟล์ ผลใช้เฉพาะโหลด ขนาด และสมมติฐานในรายงานนี้</p>`);
