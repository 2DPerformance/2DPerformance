...
(function(root){
  ...
  function build(d){
    ...
    for(const pos of centers){const cy=pos.y,k=conn.scale,B=d.steel.B,H=d.steel.H,t=d.steel.t;
      ...
      const coords=d.truss.enabled?(g.anchorPoints||g.anchorXs.map((x,i)=>({x,y:0,id:'A'+(i+1)}))):ECoords(s);
      ...
      if(d.support.type!=='hbeam'){
        const xs=[...new Set(coords.map(a=>a.x))].sort((a,b)=>a-b),ys=[...new Set(coords.map(a=>a.y))].sort((a,b)=>a-b);
        if(xs.length>1)conn.dim([xs[0],cy+ys.at(-1)],[xs.at(-1),cy+ys.at(-1)],'x',cy+ph/2+12*k,{prefix:'sx = '});
        if(ys.length>1)conn.dim([xs.at(-1),cy+ys[0]],[xs.at(-1),cy+ys.at(-1)],'y',pw/2+12*k,{prefix:'sy = '});
      }
      conn.text('PL1 '+pos.id+' / '+n(pw)+' x '+n(ph)+' x '+n(s.plate.thickness),0,cy+ph/2+(d.support.type==='hbeam'?9:23)*k,3.1*k,'center');
  ...
  function plateDetails(d){
    ...
    const forces=hbeam?[]:d.truss.enabled?(g.anchorPoints||g.anchorXs.map((x,i)=>({id:'A'+(i+1),x,y:0}))):ECoords(s);
