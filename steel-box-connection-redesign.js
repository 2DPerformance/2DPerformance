...
(function (root) {
  ...
  function arrangeWorkflow() {
    text(query('.ncy670-version'), root.NCYSC01EngineerUI ? 'UI R32 · ENGINEER' : 'UI R13 · LOCAL REVIEW');
    ...
    report.setAttribute('aria-label', doc.body.dataset.sc01DocumentsReady === 'false'
      ? '4 เอกสารยังล็อก ดูสิ่งที่ต้องแก้' : '4 ศูนย์เอกสาร');
  ...
  function sync() {
    ...
    const showingReport = stage === 'model' && ['a4', 'cad', 'plateReport', 'boq'].includes(page);
    ...
    const renderedScene = root.NCYApp?.getViewer?.()?.v6Scene;
    if (scene && (renderedScene || activeScene) && doc.activeElement !== scene) scene.value = renderedScene || activeScene.dataset.v6Scene;
    ...
    text(subtitle, root.NCYSC01EngineerUI ? 'แก้ค่า แล้วกดคำนวณ' : 'เลือกหมวด กรอกค่า แล้วคำนวณ');
  }
