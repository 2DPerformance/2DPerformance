...
function normalizeMember(member, diagnostics) {
  ...
  if (family === 'secondary-beam' && !sourceMemberId) {
...
function normalizeBeamRebarEntry(raw) {
  ...
  if (!memberId || !groupId || topLayers.length === 0 || bottomLayers.length === 0
...
function normalizeColumnRebarEntry(raw) {
  const memberId = text(raw?.memberId);
  const groupId = text(raw?.groupId);
  const basis = raw?.basis;
  ...
  if (!memberId || !groupId || Object.values(values).some(value => value === null)
    || positions.length !== values.mainBarCount || positions.some(position => position === null)) return null;
  return Object.freeze({
    key: `column:${memberId}`,
...
function normalizeFootingRebarEntry(raw) {
  const memberId = text(raw?.memberId);
  ...
  if (!memberId || !mark || Object.values(values).some(value => value === null)) return null;
  return Object.freeze({
...
export function compileNewBuildingV2RebarPathManifest(input = {}) {
  ...
  const entries = source.map(([family, raw], index) => {
    const entry = family === 'beam' ? normalizeBeamRebarEntry(raw)
      : family === 'column' ? normalizeColumnRebarEntry(raw)
        : normalizeFootingRebarEntry(raw);
