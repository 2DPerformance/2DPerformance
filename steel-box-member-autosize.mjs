...
import {preserveConcreteBounds,selectConcreteConnection} from './steel-box-member-connection-selection.mjs?v=20260913-r47';
...
export function materialMetrics(r,slot){
...
export function materialChanges(baseline,d,beforeResult,r,{A,connectionLayout}){
  ...
  for(const change of changes){change.beforeCheck=materialMetrics(beforeResult,change.slot);change.afterCheck=materialMetrics(r,change.slot);}
  if(d.support.type!=='hbeam'){
    const describe=x=>({name:x.connection.product.name,diameter:x.plate.anchorMM,grade:x.connection.productSelection.grade,hefMM:x.connection.anchorHefMM,selection:x.connection.productSelection});
    const before=describe(baseline),after=describe(d);
    if(A.stable(before)!==A.stable(after)){
      const metric=result=>{const qs=result.connections?.checks.filter(q=>['anchorSteel','productInteraction','breakoutT','bond','breakoutV','pryout','bond-sustained'].includes(q.id))||[];return {dc:qs.length?Math.max(...qs.map(q=>q.ratio)):null,pass:qs.length>0&&qs.every(numeric),deflectionMM:null};};
      changes.push({slot:'connection.product',before,after,beforeCheck:metric(beforeResult),afterCheck:metric(r)});
    }
  }
  if(connectionLayout){
    const describe=x=>x.support.type==='hbeam'?{width:connectionLayout(x).width,height:connectionLayout(x).height,anchorsApplicable:false}:({...connectionLayout(x),rows:x.truss.enabled&&x.connection.anchorLayout==='legacy-face'?1:x.connection.anchorRows,cols:x.connection.anchorCols,diameter:x.plate.anchorMM,edgeX:x.connection.anchorEdgeXMM,edgeY:x.connection.anchorEdgeYMM,concreteEdges:['edgeLeftMM','edgeRightMM','edgeTopMM','edgeBottomMM'].map(k=>x.connection[k])});
    const before=describe(baseline),after=describe(d);
    if(A.stable(before)!==A.stable(after)){const fit=r.connections?.checks.filter(x=>['hardware-fit','geometry','support-fit',...(d.support.type==='hbeam'?['plate']:[])].includes(x.id))||[];changes.push({slot:'connection.layout',before,after,beforeCheck:{dc:null,pass:false,deflectionMM:null},afterCheck:{dc:null,pass:fit.length>0&&fit.every(numeric),deflectionMM:null}});}
  }
  return changes;
}
...
export async function selectMaterials(original,{A,C,records,weldGeometry,connectionLayout,initial,lockMembers=false,membersOnly=false,active=()=>true,progress=()=>{},yieldTask=()=>new Promise(r=>setTimeout(r,0)),maxTrials=500,maxMilliseconds=60000}){
  const started=Date.now(),baseline=A.clone(original),d=A.clone(original);let count=0,r=initial||C.calculate(d),member=membersOnly?A.calculate(d):r;
  const note={status:'unchanged',trials:0,changes:[],remaining:[],input:baseline,result:r};
  if((membersOnly?member:r).errors?.length&&(!d.truss.enabled||A.validate(d).length||!membersOnly&&C.validate(d).length||A.validate(d,{geometryOnly:true}).length))return {...note,status:'invalid'};
  ...
  if(lockMembers&&!(r.memberReady??r.ready))return {...note,status:'unavailable',remaining:['เหล็กที่เลือกยังไม่ผ่านกำลังหรือการแอ่น จึงคงขนาดที่เลือกและให้แก้ก่อนตรวจจุดต่อ']};
  try{
    ...
    if(!lockMembers&&(!(member.memberReady??member.ready)||!weldFeasible())){
      if(!d.truss.enabled){
    ...
    if(membersOnly){
      // The interactive picker checks connection products in its next step.
      // A small existing anchor must not exhaust this member search first.
      await tick('ตรวจชุดเหล็กก่อนเลือกเพลทและจุดยึด');r=C.calculate(d);count++;
      const changes=materialChanges(baseline,d,note.result,r,{A,connectionLayout});
