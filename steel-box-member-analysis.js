...
(function(root){
  ...
  const VERSION='r47', SCHEMA='sc01.member-inputs.v7', MODULUS=200000, D=root.NCYSC01MemberLoads,Steel=root.NCYSC01SteelDesign;
  ...
  // New detail policies are explicit and versioned; migrated jobs keep their
  // original dimensions and conservative reserved hardware envelope.
  const legalReferences=Object.freeze({
    'th-2566-2567':{label:'กฎกระทรวง 2566 + ประกาศ มท. 30 ส.ค. 2567',url:'https://ratchakitcha.soc.go.th/documents/140A054N0000000000400.pdf',additionalUrl:'https://www.reic.or.th/Upload/9_52591_1725415347_23859.pdf'},
    'th-2566':{label:'กฎกระทรวงโครงสร้างและวัสดุ พ.ศ. 2566',url:'https://ratchakitcha.soc.go.th/documents/140A054N0000000000400.pdf'},
    'project':{label:'ข้อกำหนดเฉพาะโครงการ',url:''},
    'legacy':{label:'งานเดิม · ยังไม่ได้ระบุเอกสารกฎหมาย',url:''}
  });
  const washerSource='https://marketplacemedia.witglobal.net/source/marketplace/stmedia/wuerth/documents/documents/std.lang.all/29208645.pdf';
  const washerTable=Object.freeze({8:[8.4,16,1.6],10:[10.5,20,2],12:[13,24,2.5],16:[17,30,3],20:[21,37,3],24:[25,44,4],27:[28,50,4],30:[31,56,4]});
  const inchCeil=x=>Math.round(Math.ceil((x-1e-9)/25.4)*254)/10;
  const plateMinimum=d=>inchCeil(Math.max(d.steel.H,d.steel.B)+50.8);
  function hardware(d){
    const a=d.plate.anchorMM,row=washerTable[a];
    if(d.connection.hardware==='iso7089'&&row)return {id:'iso7089',insideMM:row[0],outsideMM:row[1],thicknessMM:row[2],radius:row[1]/2,accessRadius:Math.max(row[1]/2,1.25*a),allowanceMM:2,source:washerSource};
    const radius=Math.max(18,1.25*a);return {id:'reserved',outsideMM:2*radius,radius,accessRadius:radius,allowanceMM:0,source:''};
  }
  function detailLayout(d,{rows=d.connection.anchorRows,cols=d.connection.anchorCols,weld=d.connection.weldSizeMM,extra=0}={}){
    const c=d.connection,a=d.plate.anchorMM,h=hardware(d),min=plateMinimum(d),edge=Math.max(1.5*a,h.radius+h.allowanceMM);
    const source=root.NCYSC01DesignProducts.row(d),spacing=source?.product.minSpacing??(c.productSelection.id==='manual'&&c.product.minSpacing>0?c.product.minSpacing:5*a);
    if(c.anchorLayout==='outside-hss'){
      const edge=Math.max(20,1.5*a,h.radius+h.allowanceMM),access=Math.max(25.4,h.radius+weld+h.allowanceMM,h.accessRadius+h.allowanceMM);
      const width=inchCeil(Math.max(min,d.steel.B+2*(access+edge),(cols-1)*spacing+2*edge)+extra);
      const height=inchCeil(Math.max(min,rows===1?d.steel.H+50.8:d.steel.H+2*(access+edge),(rows-1)*spacing+2*edge)+extra);
      return {width,height,edgeX:edge,edgeY:rows===1?height/2:edge,minimumMM:min};
    }
    const pitch=(cols-1)*spacing,access=Math.max(h.radius+weld+h.allowanceMM,h.accessRadius);
    const needX=d.truss.enabled?d.steel.B+2*(access+edge):pitch+2*edge;
    const needY=d.truss.enabled?d.steel.H+50.8:Math.max(d.steel.H+2*(access+edge),(rows-1)*spacing+2*edge);
    // Begin square; enlarge only the direction that actually needs room.
    // Keeping an unnecessary lateral overhang would require thicker steel.
    const width=inchCeil(Math.max(min,needX)+(d.truss.enabled?extra:0)),height=inchCeil(Math.max(min,needY)+(d.truss.enabled?0:extra));
    return {width,height,edgeX:d.truss.enabled?edge:(width-pitch)/2,edgeY:d.truss.enabled?height/2:edge,minimumMM:min};
  }
  function plateGeometry(d){
    const c=d.connection;
    if(!c.plateAuto)return {width:c.plateWidthMM,height:c.plateHeightMM,edgeX:c.anchorEdgeXMM,edgeY:c.anchorEdgeYMM};
    if(c.plateSizing==='square-inch'||c.anchorLayout==='outside-hss'&&d.support.type!=='hbeam')return d.support.type==='hbeam'?{width:plateMinimum(d),height:plateMinimum(d),edgeX:c.anchorEdgeXMM,edgeY:c.anchorEdgeYMM}:detailLayout(d);
    return {width:d.steel.B+d.plate.allowanceMM,height:d.steel.H+d.plate.allowanceMM,edgeX:c.anchorEdgeXMM,edgeY:c.anchorEdgeYMM};
  }
  const localDate=()=>{const d=new Date();return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`;};
  const fresh=()=>({schema:SCHEMA,project:{name:'',number:'',designer:'',date:localDate()},
    ...
    connection:{anchorLayout:'outside-hss',plateSizing:'square-inch',hardware:'iso7089',anchorPreference:'4',plateAuto:true,plateWidthMM:100.8,plateHeightMM:150.8,plateFy:245,plateFu:400,
      ...
      productSelection:root.NCYSC01DesignProducts.fresh(),
    ...
    design:{legalBasis:'th-2566-2567',profile:'aisc360-22_aci318-19',forceUnit:'kN',stressUnit:'MPa',custom:Object.fromEntries(Object.entries(M.defaultState().customFactors).filter(([k])=>k.startsWith('phi')))},
  ...
  const shape=fresh();
  const v6Shape=clone(shape);delete v6Shape.connection.anchorLayout;v6Shape.schema='sc01.member-inputs.v6';
  const priorShape=clone(v6Shape);for(const k of ['plateSizing','hardware','anchorPreference'])delete priorShape.connection[k];delete priorShape.design.legalBasis;priorShape.schema='sc01.member-inputs.v5';
  ...
  function validate(d,{geometryOnly=false}={}){
    ...
    if(!['outside-hss','legacy-face'].includes(d.connection.anchorLayout))errors.push({path:'connection.anchorLayout',message:'เลือกรูปแบบตำแหน่งพุก'});
    ...
    for(const [path,allowed] of [['connection.plateSizing',['square-inch','legacy-rectangle']],['connection.hardware',['iso7089','reserved']],['connection.anchorPreference',['auto','4','2','manual']],['design.legalBasis',Object.keys(legalReferences)]]){const [a,b]=path.split('.');if(!allowed.includes(d[a][b]))errors.push({path,message:'เลือกค่ารายละเอียดที่รองรับ'});}
    if(d.support.type!=='hbeam'&&d.connection.hardware==='iso7089'&&!washerTable[d.plate.anchorMM])errors.push({path:'plate.anchorMM',message:'ไม่มีขนาดแหวน ISO 7089 สำหรับพุกนี้ในรายการที่ตรวจสอบ'});
  ...
  function migrate(value){
    const d=clone(value);
    if(d?.schema===v6Shape.schema){if(!validDTO(d,v6Shape))throw Error('ข้อมูลรุ่น v6 ไม่ครบหรือมีฟิลด์ที่ไม่รู้จัก');d.connection.anchorLayout='legacy-face';d.schema=SCHEMA;}
    if(d?.schema==='sc01.member-inputs.v4'){
      const prior=clone(priorShape);delete prior.connection.productSelection;prior.schema=d.schema;
      if(!Object.hasOwn(d.support||{},'hbeam'))delete prior.support.hbeam;
      if(!validDTO(d,prior))throw Error('ข้อมูลตรวจเหล็กรุ่น v4 ไม่ครบหรือมีฟิลด์ที่ไม่รู้จัก');
      d.connection.productSelection={...root.NCYSC01DesignProducts.fresh(),id:'manual'};
      d.support.hbeam??=clone(shape.support.hbeam);d.schema=priorShape.schema;
    }
    if(d?.schema===priorShape.schema&&!Object.hasOwn(d.support||{},'hbeam')){const prior=clone(priorShape);delete prior.support.hbeam;if(!validDTO(d,prior))throw Error('ข้อมูลรุ่นก่อนเพิ่มหน้าตัด H-beam ไม่ครบ');d.support.hbeam=clone(shape.support.hbeam);}
    if(d?.schema===priorShape.schema){if(!validDTO(d,priorShape))throw Error('ข้อมูลรุ่น v5 ไม่ครบหรือมีฟิลด์ที่ไม่รู้จัก');Object.assign(d.connection,{anchorLayout:'legacy-face',plateSizing:'legacy-rectangle',hardware:'reserved',anchorPreference:'manual'});d.design.legalBasis='legacy';d.schema=SCHEMA;}
    if(['sc01.member-inputs.v1','sc01.member-inputs.v2','sc01.member-inputs.v3'].includes(d?.schema)){
      const old=clone(priorShape);delete old.connection;delete old.design;old.support={type:'concrete-wall'};delete old.geometry.purlinLayout;old.schema=d.schema;
      ...
      d.schema=SCHEMA;d.support={...clone(shape.support),...d.support};d.connection=clone(shape.connection);d.design=clone(shape.design);d.geometry.purlinLayout='equal';Object.assign(d.connection,{anchorLayout:'legacy-face',plateSizing:'legacy-rectangle',hardware:'reserved',anchorPreference:'manual'});d.design.legalBasis='legacy';
    }
  ...
  function fromLegacy(raw){
    ...
    Object.assign(d.connection,{anchorLayout:'legacy-face',plateSizing:'legacy-rectangle',hardware:'reserved',anchorPreference:'manual',plateAuto:false,plateWidthMM:d.truss.enabled?v.v62.rootPlateWidthMM:v.plate.width,plateHeightMM:d.truss.enabled?v.v62.rootPlateHeightMM:v.plate.height,plateFy:v.plate.Fy,plateFu:v.plate.Fu,
    ...
    d.design.legalBasis=Object.hasOwn(legalReferences,v.v659?.profiles?.legalProfile)?v.v659.profiles.legalProfile:'legacy';
  ...
  root.NCYSC01MemberAnalysis=Object.freeze({VERSION,SCHEMA,legalReferences,washerSource,hardware,inchCeil,plateMinimum,detailLayout,plateGeometry,fresh,stable,clone,validDTO,validate,migrate,layout,nativeState,beamColumn,beamWithPoints,cableCase,calculate,reportAllowed,fromLegacy});
