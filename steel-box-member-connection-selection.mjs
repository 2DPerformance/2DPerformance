...
export async function selectConcreteConnection(d,{reference,A,C,records,layout,weldGeometry,evaluate}){
  ...
  const outside=c.anchorLayout==='outside-hss',counts=outside?[[2,2]]:[[c.anchorRows,c.anchorCols]];
  ...
  if(!outside){if(saved.truss.enabled){for(const cols of [2,4,6,8])if(cols>c.anchorCols)counts.push([c.anchorRows,cols]);}
  ...
  for(const [rows,cols] of counts){
    if(outside){for(const extra of [0,25.4,50.8,101.6]){const g=A.detailLayout(saved,{rows,cols,weld:Math.max(8,c.weldSizeMM),extra});geometries.push({rows,cols,...g,edge:g.edgeX});}continue;}
  ...
  for(const geometry of geometries){
    ...
    if(geometry)Object.assign(cc,{plateAuto:false,plateWidthMM:geometry.width,plateHeightMM:geometry.height,anchorRows:geometry.rows,anchorCols:geometry.cols,anchorEdgeXMM:geometry.edge,anchorEdgeYMM:outside?geometry.edgeY:saved.truss.enabled?c.anchorEdgeYMM:geometry.edge});
