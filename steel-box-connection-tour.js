...
(function (root) {
  ...
  function start() {
    if (root.NCYSC01MemberWorkflow?.active) return root.NCYSC01MemberTour?.start() === true;
  ...
  const route = fn => (...args) => root.NCYSC01MemberWorkflow?.active ? root.NCYSC01MemberTour?.[fn](...args) : ({close,restart,next,previous,minimize,locate,getState:state})[fn](...args);
  root.NCYSC01Tour = Object.freeze({ VERSION: '20260914-r48', get STEPS(){return root.NCYSC01MemberWorkflow?.active ? root.NCYSC01MemberTour?.STEPS : STEPS;}, start, close:route('close'), restart:route('restart'), next:route('next'), previous:route('previous'), minimize:route('minimize'), locate:route('locate'), getState:route('getState') });
})(window);
