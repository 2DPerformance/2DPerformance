...
(function(root){
  ...
  const rootAnchorPoints=g=>g.anchorPoints||g.anchorXs.map((x,i)=>({id:'A'+(i+1),x,y:0}));
  function nativeState(d){
    const {s}=A().nativeState(d),a=d.plate.anchorMM,c=d.connection,pg=A().plateGeometry(d),hardware=A().hardware(d);
    s.r47AnchorLayout=c.anchorLayout;
    Object.assign(s.plate,{width:pg.width,height:pg.height,thickness:d.plate.thicknessMM,Fy:c.plateFy,Fu:c.plateFu,holeDiameterMM:a+2,standOffMM:0});
    ...
    Object.assign(s.anchors,{type:c.anchorType,diameter:a,Ase:c.anchorAseMM2,futa:c.anchorFuta,hef:c.anchorHefMM,cols:c.anchorCols,rows:c.anchorRows,plateEdgeX:pg.edgeX,plateEdgeY:pg.edgeY});
    if(hardware.id==='iso7089')s.r45Hardware=hardware;
    ...
    // Legacy roots retain their side row. New roots use the same four local
    // coordinates as the scoped connection calculation and drilling schedule.
    Object.assign(s.v62,{autoFitRootDepth:false,rootPlateWidthMM:c.anchorLayout==='legacy-face'&&c.plateAuto&&c.plateSizing==='legacy-rectangle'?Math.max(s.plate.width,d.steel.B+7*a+8):s.plate.width,
    ...
    if(d.truss.enabled){
      ...
      const anchorXs=anchorCols===1?[0]:Array.from({length:anchorCols},(_,i)=>-plateW/2+pg.edgeX+i*(plateW-2*pg.edgeX)/(anchorCols-1));
      const anchorPoints=c.anchorLayout==='outside-hss'?root.NCYEngine.anchorCoordinates(s).anchors:anchorXs.map((x,i)=>({id:'A'+(i+1),x,y:0}));
      s.v62.r37Geometry={supportH:H,H,requestedDepthMM:depth,effectiveDepthMM:depth,maxDepthMM:H-2*edge-plateH,plateW,plateH,plateT:s.plate.thickness,gap:depth-plateH,edge,top,bottom,supportTop,supportBottom,
        fits:top.y1<=supportTop&&bottom.y0>=supportBottom&&depth>plateH,adjusted:false,anchorCols,anchorXs,anchorPoints,anchorsPerPlate:anchorPoints.length,totalAnchors:anchorPoints.length*2};
    }
    return s;
  }
  function connectionFootprint(d){
    const s=nativeState(d),g=root.NCYV62.rootGeometry(s),points=d.truss.enabled?rootAnchorPoints(g):root.NCYEngine.anchorCoordinates(s).anchors;
  ...
  function rootActions(d,r,index=-1){
    ...
    const plates=centers.map(p=>{
      const coordinates=d.truss.enabled?rootAnchorPoints(g):root.NCYEngine.anchorCoordinates(s).anchors;
  ...
  function nativeResult(d,r=null){
    ...
    const out=A().clone(geometryResult);out.scopedActions=true;out.scopedHbeam=d.support.type==='hbeam';out.scopedVerified=!!root.NCYSC01MemberConnections?.reportAllowed(r,d);
    ...
    out.cases=r.cases.map((c,i)=>{
      ...
      if(d.truss.enabled){const row={caseName:c.name};for(const side of ['top','bottom']){const p=data.plates.find(p=>p.id===side.toUpperCase());row[side]={name:side,action:{Nx:p.N,Vy:p.Vy},T:Math.max(0,p.N),C:Math.max(0,-p.N),V:Math.abs(p.Vy),Tper:Math.max(0,p.N)/p.anchors.length,Vper:Math.abs(p.Vy)/p.anchors.length};}rootCases.push(row);}
        ...
        const actual=r.connections?.all.find(x=>x.caseIndex===i&&x.plateId===data.plates[0].id);
        if(actual?.group){q.group=A().clone(actual.group);q.normal=A().clone(actual.normal);q.scopedConnectionMethod=actual.method;for(const check of actual.checks)q[check.id]=A().clone(check.detail||check);}
  ...
  function rootSVG(d){
    ...
    const coords=tr?rootAnchorPoints(g):root.NCYEngine.anchorCoordinates(s).anchors;
