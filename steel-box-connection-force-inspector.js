...
(function(root) {
  ...
  function trussForceData(result,index){
    ...
    const points=g?.anchorPoints||g?.anchorXs?.map(x=>({x,y:0}));
    if(!g||!Array.isArray(rows)||!points?.length||points.some(a=>!finite(a.x)||!finite(a.y))||!finite(g.top?.cy)||!finite(g.bottom?.cy))return fail('ยังไม่มีรูปทรงและแรงปฏิกิริยาเพลทราก Truss');
    const cases=[];
    for(const c of result.cases){
      ...
      for(const side of ['top','bottom']){
        const r=found[0][side],cy=g[side].cy,n=points.length;
        ...
        points.forEach((a,i)=>anchors.push({id:side.toUpperCase()+'-A'+(i+1),side,number:i+1,x:a.x,y:cy+a.y,T:r.Tper,Vx:0,Vy:r.action.Vy/n,V:r.Vper}));
      }
  ...
  function render(fit=false) {
    ...
    if(provider)message.textContent=`${data.caseName} · แรงจากผลคำนวณชุดปัจจุบัน · ${source.scopedVerified?'✓ สมาชิกและจุดต่อผ่านครบ':'ผลตรวจจุดต่อยังไม่ครบ / ไม่ผ่าน ดูรายละเอียดในผลคำนวณ'}${layer==='contact'?' · แรงกดบล็อกสมดุล DG1 ไม่ใช่ความเค้น FEM':''}`;
  ...
  function openInspector(adapter,{host=null}={}) {
    ...
    if(available.ok){
      if(provider){if(!source.cases.every(c=>['dg1-2024-aci318-19-r44','dg1-sti2024-aci318-19-r47'].includes(c.scopedConnectionMethod)))byId('sc01ForceLayer').querySelector('[value="contact"]')?.remove();if(source.scopedHbeam){for(const option of [...byId('sc01ForceLayer').options])if(option.value!=='all')option.remove();byId('sc01ForceLayer').options[0].textContent='แรงที่แนวเชื่อมเพลทกับ H-beam';}}
      if(available.truss){
