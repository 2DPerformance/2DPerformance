...
(function (root) {
  ...
  function weldTrial(state, before) {
    ...
    const range=weldRange(before);
    if(range.known&&range.max<range.min)throw Error(`ขารอยเชื่อมต้องไม่น้อยกว่า ${range.min} mm แต่หน้าตัดนี้ยอมได้ไม่เกิน ${range.max} mm จึงไม่มีขนาดในช่วงร่วม ตรวจความหนาหน้าตัดและเพลทก่อนทดลองใหม่`);
  ...
  function weldRange(result){
    const cases=result?.cases||[];
    if(!cases.length||!cases.every(c=>[c.weld?.minSize,c.weld?.maxSize].every(finite)))return {known:false};
    return {known:true,min:Math.max(...cases.map(c=>c.weld.minSize)),max:Math.min(...cases.map(c=>c.weld.maxSize))};
  }
  root.NCYSC01CalculationAudit = Object.freeze({VERSION:'r31', status, coverage, weldTrial, weldRange});
