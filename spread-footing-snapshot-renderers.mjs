...
// Owner 2026-09-25: bar labels print the standard spacing (@250/@200/@150/@100
// mm) the engine verified at that spacing; equations keep the actual spacing.
const printedSpacingValue = (bar, keys = ['spacingMm', 'spacing', 'spacingDisplay']) => {
  const nominal = firstValue(bar, ['nominalSpacingMm'], EMPTY)
  return numericValue(nominal) === null ? firstValue(bar, keys, EMPTY) : nominal
}
...
const renderPlanRebarCallout = (bar, direction, y) => {
  ...
  const printedSpacing = printedSpacingValue(bar)
  const heading = `${mark} · ${presentation.direction}/${presentation.axis} ${presentation.axisSymbol}`
  const specification = `${presentation.layer} · ${count}-${size} @ ${rawFormattedValueWithUnit(printedSpacing, 'mm', 1)}`
...
const renderSectionSvg = (
  ...
  const printedA = printedSpacingValue(barA)
  const printedB = printedSpacingValue(barB)
  ...
  return `<svg class="sf-section-svg" viewBox="0 0 620 360" role="img"
    ...
    <g class="sf-section-labels">
      <text x="88" y="286" fill="#c2410c" data-section-direction="A" data-axis="X"
        ...
        )}-${reinforcementDesignation(barA)} @ ${rawFormattedValueWithUnit(printedA, 'mm', 1)}`, 360, 44)}>
        ...
        @ ${formattedValueWithUnit(printedA, 'mm', 1)}</text>
      <text x="88" y="318" fill="#0f766e" data-section-direction="B" data-axis="Y"
        ...
        )}-${reinforcementDesignation(barB)} @ ${rawFormattedValueWithUnit(printedB, 'mm', 1)}`, 360, 44)}>
        ...
        @ ${formattedValueWithUnit(printedB, 'mm', 1)}</text>
  ...
  const callout = (bar, direction, y, color) => {
    ...
    return `<g class="a4-compact-callout a4-compact-callout--leader"
      ...
      <text x="224" y="${safeNumber(y + 42)}"${fittedSvgTextLength(
        specification,
        180,
        // 24 px glyphs: ~13 characters fill the 180-unit column; a longer
        // spacing text ("5-DB12 @ 234.5 mm") ran past the 420-unit viewBox.
        13
      )}>${escapeHtml(specification)}</text>
    </g>`
  }
  ...
  const selectedRebarInline = (direction, bar) => {
    ...
    const aggregateStatus =
      ...
      !relatedChecks.every((record) => displayStatus(record) === 'PASS')
        ? hasDirectionalFailure
          ...
          : NOT_EVALUATED
        : 'PASS'
    const actualSpacingMm = firstValue(bar, ['spacingMm', 'spacing'], EMPTY)
    const spacingMm = printedSpacingValue(bar, ['spacingMm', 'spacing'])
    const spacingCm =
      numericValue(spacingMm) === null ? EMPTY : numericValue(spacingMm) / 10
    const actualNote = numericValue(spacingMm) !== null && numericValue(actualSpacingMm) !== null
      && Math.abs(numericValue(spacingMm) - numericValue(actualSpacingMm)) > 0.05
      ? ` · ระยะเฉลี่ยจริง ${formattedValueWithUnit(actualSpacingMm, 'mm', 1)}` : ''
    return `<p class="${statusClass(aggregateStatus)}"
      ...
      <b>${barCount(bar)}-${metricText(reinforcementDesignation(bar))}
        ...
        ${spacingCm === EMPTY ? '' : `(≈ ${formattedValueWithUnit(spacingCm, 'cm', 1)}${actualNote})`}</b>
              ...
              <section><h6>ทิศ A</h6>${factList([
                ...
                [
                  ...
                  `${metricText(reinforcementSummary(barA, 'BAR-A'))} @ ${formattedValueWithUnit(
                    printedSpacingValue(barA, ['spacingMm', 'spacing']),
                    'mm',
                    1
                  )}`,
                ],
              ...
              <section><h6>ทิศ B</h6>${factList([
                ...
                [
                  ...
                  `${metricText(reinforcementSummary(barB, 'BAR-B'))} @ ${formattedValueWithUnit(
                    printedSpacingValue(barB, ['spacingMm', 'spacing']),
                    'mm',
                    1
                  )}`,
                ],
