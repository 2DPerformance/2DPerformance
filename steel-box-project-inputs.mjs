...
const materials = window.NCYSC01MaterialInputs;
const reportProfile = window.NCYSC01ReportProfile;
// Choice metadata is optional, including when this module starts with a selected
// SKU. Older projects have the exact same input contract without these names.
for (const key of materials?.metadataKeys || []) delete inputShape.v6[key];
for (const key of Object.keys(reportProfile?.extraFields || {})) delete inputShape.meta[key];
const optionalFields = { v57:{loadOrigin:'string'}, v5:{exportedAt:'string'},
  meta:Object.fromEntries(Object.keys(reportProfile?.extraFields || {}).map(key => [key,'string'])),
  v6:Object.fromEntries((materials?.metadataKeys || []).map(key => [key,'string'])) };
...
function captureCommitted() {
  const input = withoutOutputCaches(app.getState());
  return materials ? materials.cleanMetadata(input) : input;
}
function capture() {
  if (window.NCYSC01MaterialSelection?.hasPending?.()) throw new Error('มีวัสดุที่เลือกไว้ กดคำนวณและดูผลหรือยกเลิกการเลือกก่อนบันทึก');
  return captureCommitted();
}
function validate(input) {
  if (!matchesShape(input,inputShape) || materials && !materials.validMetadata(input) || reportProfile && !reportProfile.validMetadata(input)) return false;
  try { const restored=model.importState(input); return model.validate(restored).length===0; } catch (_) { return false; }
}
function apply(input) {
  if (window.NCYSC01MaterialSelection?.hasPending?.()) throw new Error('คำนวณหรือยกเลิกวัสดุที่เลือกก่อนเปิดงานอื่น');
...
window.SVSteelBoxProjectInputs = Object.freeze({ capture, captureCommitted, validate, apply });
...
} else if (!window.NCYSC01MemberAnalysis || new URLSearchParams(location.search).get('qa')==='1' && new URLSearchParams(location.search).get('legacy')==='1') {
