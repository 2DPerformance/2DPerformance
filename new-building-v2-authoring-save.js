...
export function mountP1AuthoringSave({ api, storage, identity, getSnapshot, restore, canRestore, toast, onResult, onChange, host = window }) {
  ...
  function render() {
    ...
    const readableError = /[ก-๙]/.test(error) ? error : 'กดบันทึกงานเพื่อดูรายละเอียด';
    status.textContent = error ? `ยังไม่บันทึกร่าง · ${readableError} · กด ↓ JSON เพื่อสำรอง` : !ready ? 'กำลังเปิดร่างที่บันทึกไว้…' : dirty ? 'มีการแก้ไขใหม่ · กำลังบันทึกร่างในเครื่อง…' : saved ? '✓ บันทึกร่างในเครื่องแล้ว' : 'ร่างแปลน · ยังไม่มีการแก้ไขใหม่';
    status.title = error || status.textContent;
  ...
  function capture() {
    ...
    const inputs = getSnapshot();
    return api.createNewBuildingV2AuthoringDraft(project, inputs, { basisDrawingFingerprint:
      typeof inputs.sourceDrawingFingerprint === 'string' ? inputs.sourceDrawingFingerprint : current.p1DrawingFingerprint || '' });
  }
  ...
  async function restoreDraft(draft, keepUndo = false) {
    // Recovery inputs intentionally omit output pointers. Preserve their checked
    // editing basis separately for the next compare-and-swap, not as authority.
    await restore(draft.inputs, { keepUndo, basisDrawingFingerprint: draft.basisDrawingFingerprint });
  ...
  async function initialize() {
    ...
    initialization = (async () => {
      ...
      if (inspected) {
        if (inspected.draft.basisDrawingFingerprint === (project.p1DrawingFingerprint || '') && canRestore()) {
          await restoreDraft(inspected.draft); restored = true;
        } else {
          conflict = true;
          ...
          recover.addEventListener('click', async () => {
            await restoreDraft(inspected.draft, true); conflict = false; error = ''; recover.remove(); schedule();
          });
    ...
    })().catch(failure => { initialization = null; error = failure?.message || 'เปิดระบบบันทึกไม่ได้'; render(); throw failure; });
    return initialization;
  }
  function current() {
    if (!ready || conflict || error) return false;
    try {
      const destination = storage();
      const currentProject = checkProject(destination);
      const stored = JSON.parse(destination.getItem(key) || 'null');
      const inspected = api.inspectNewBuildingV2AuthoringDraft(currentProject, stored);
      const inputs = api.authoringDraftInputs(getSnapshot());
      return Boolean(inspected.ok
        && stored.basisDrawingFingerprint === (currentProject.p1DrawingFingerprint || '')
        && JSON.stringify(stored.inputs) === JSON.stringify(inputs)
        && writer?.current(inputs));
    } catch { return false; }
  }
  function fileFeedback(message, file, durable, issues = []) {
    if (onResult) onResult({ operation: 'file', file, draftSaved: Boolean(durable),
      drawingCommitted: false, current: Boolean(durable && current()),
      reason: file.kind === 'failed' ? 'FILE_WRITE_FAILED' : '', issues });
    else toast(message);
  }
  ...
  async function saveFile(forceDownload = false) {
    ...
    try {
      ...
      durable = await localSave;
      if (chosen.kind === 'cancelled') {
        fileFeedback(durable ? 'ยกเลิกเลือกไฟล์ · ร่างบันทึกในเครื่องแล้ว · ใช้ ↓ JSON เพื่อดาวน์โหลดสำรองได้' : 'ยกเลิกเลือกไฟล์ · ร่างยังไม่บันทึก โปรดกด ↓ JSON เพื่อดาวน์โหลดสำรอง', { kind: 'cancelled' }, durable, error ? [error] : []);
        return;
      }
      ...
      if (result.kind === 'failed') {
        ...
        fileFeedback('เขียนไฟล์ที่เลือกไม่สำเร็จ · ส่งไฟล์สำรองไปที่รายการดาวน์โหลดแล้ว', { kind: 'download', name: filename, fallback: true }, durable, [result.error, error].filter(Boolean));
      } else fileFeedback(result.kind === 'file' ? `บันทึกไฟล์ ${result.name} แล้ว · เปิดกลับได้จากรายการโครงการ → เปิดไฟล์`
        : 'ส่งไฟล์ไปที่รายการดาวน์โหลดแล้ว · ตรวจไฟล์ .json ในโฟลเดอร์ดาวน์โหลด',
        { kind: result.kind, name: result.name || filename, fallback: Boolean(chosen.reason) }, durable, [chosen.reason, error].filter(Boolean));
    } catch (failure) { fileFeedback('บันทึกไฟล์ไม่สำเร็จ · โปรดลองอีกครั้ง', { kind: 'failed' }, durable, [failure?.message || 'บันทึกไฟล์ไม่สำเร็จ']); }
  ...
  return { initialize, schedule, flush, current, get ready() { return ready; }, get error() { return error; }, get busy() { return busy; } };
}
