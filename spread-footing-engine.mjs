...
export const SPREAD_FOOTING_AUTO_DESIGN_POLICY = deepFreeze({
  ...
  // Owner 2026-09-25: bottom bars are specified at a standard spacing only
  // (@25 / @20 / @15 / @10 cm). Auto picks the widest one whose steel area at
  // that spacing already meets the requirement; the count follows from it.
  standardSpacingsMm: [250, 200, 150, 100],
...
function buildCalculationResults(input) {
  ...
  // Standard spacing to print (@25/@20/@15/@10 cm): the smallest standard value
  // not below the actual centre-to-centre spacing whose steel area, taken at
  // that spacing, still meets the requirement — safe even if bars are laid at
  // the printed spacing. Otherwise none (labels keep the actual spacing).
  for (const [rebar, flex] of [[reinforcementA, flexure.A], [reinforcementB, flexure.B]]) {
    rebar.nominalSpacingMm = standardLabelSpacingMm(rebar, flex?.asRequiredMm2PerM, footing.thicknessM * 1000);
  }
...
function spacingLimitMm(thicknessMm) {
  return Math.min(SPREAD_FOOTING_AUTO_DESIGN_POLICY.maximumSpacingMm, 3 * thicknessMm);
}

function standardLabelSpacingMm(rebar, requiredAreaMm2PerM, thicknessMm) {
  if (!Number.isFinite(rebar?.spacingMm) || !Number.isFinite(rebar?.diameterMm)) return null;
  const barArea = barAreaMm2(rebar.diameterMm);
  const ascending = [...SPREAD_FOOTING_AUTO_DESIGN_POLICY.standardSpacingsMm].sort((a, b) => a - b);
  const found = ascending.find((spacing) => spacing + EPSILON >= rebar.spacingMm
    && spacing <= spacingLimitMm(thicknessMm) + EPSILON
    && (!Number.isFinite(requiredAreaMm2PerM) || barArea * 1000 / spacing + EPSILON >= requiredAreaMm2PerM));
  return found ?? null;
}

// Widest standard spacing (below `belowMm`) whose area at that spacing meets
// the requirement and the maximum-spacing limit; null when none does.
function autoStandardSpacingMm({ requiredAreaMm2PerM, diameterMm, thicknessMm, belowMm = Number.POSITIVE_INFINITY }) {
  if (!Number.isFinite(requiredAreaMm2PerM) || requiredAreaMm2PerM < 0) return null;
  const barArea = barAreaMm2(diameterMm);
  const descending = [...SPREAD_FOOTING_AUTO_DESIGN_POLICY.standardSpacingsMm].sort((a, b) => b - a);
  return descending.find((spacing) => spacing < belowMm - EPSILON
    && spacing <= spacingLimitMm(thicknessMm) + EPSILON
    && barArea * 1000 / spacing + EPSILON >= requiredAreaMm2PerM) ?? null;
}

function autoCountForSpacing({ distributionWidthMm, coverMm, diameterMm, spacingMm }) {
  const usableWidthMm = distributionWidthMm - 2 * coverMm - diameterMm;
  if (!(usableWidthMm > 0) || !(spacingMm > 0)) return Number.POSITIVE_INFINITY;
  return Math.max(2, Math.ceil(usableWidthMm / spacingMm - EPSILON) + 1);
}
...
function evaluateAutoRebarCandidate(baseInput, geometry, diameterMm) {
  ...
  let spacingA = autoStandardSpacingMm({ requiredAreaMm2PerM: trialResults.flexure.A.asRequiredMm2PerM,
    diameterMm, thicknessMm });
  let spacingB = autoStandardSpacingMm({ requiredAreaMm2PerM: trialResults.flexure.B.asRequiredMm2PerM,
    diameterMm, thicknessMm });
  if (spacingA === null || spacingB === null) {
    return { ok: false, validation: trialValidation, results: trialResults };
  }
  const countFor = (side, spacingMm) => autoCountForSpacing({
    distributionWidthMm: (side === 'A' ? geometry.footingYM : geometry.footingXM) * 1000,
    coverMm, diameterMm, spacingMm });
  let countA = countFor('A', spacingA);
  let countB = countFor('B', spacingB);
  ...
  for (let iteration = 0; iteration < 198; iteration += 1) {
    ...
    // Step to the next smaller standard spacing (never an odd spacing).
    const stepDown = (spacing) => [...SPREAD_FOOTING_AUTO_DESIGN_POLICY.standardSpacingsMm]
      .sort((a, b) => b - a).find((candidate) => candidate < spacing - EPSILON) ?? null;
    if (increaseA) { spacingA = stepDown(spacingA); if (spacingA === null) return { ok: false, validation, results }; countA = countFor('A', spacingA); }
    if (increaseB) { spacingB = stepDown(spacingB); if (spacingB === null) return { ok: false, validation, results }; countB = countFor('B', spacingB); }
...
/**
 * Synchronous calculation for host apps that already hold this module and
 * must size footings inside a synchronous analysis pass. Same validation and
 * calculation core as createSpreadFootingSnapshot, but without fingerprints,
 * payload hash or snapshot id, so it is calculation evidence for the host and
 * never a Calculation Snapshot. Reports must still render a snapshot.
 */
export function evaluateSpreadFootingDraft(draft = {}) {
  const validation = validateSpreadFootingDraft(draft);
  if (!validation.ok) {
    return deepFreeze({ ok: false, status: 'HOLD', errors: validation.errors, issues: validation.issues });
  }
  try {
    return deepFreeze({
      ok: true,
      input: validation.input,
      designSelection: spreadFootingAutoDesignSelection(validation.input),
      results: buildCalculationResults(validation.input),
    });
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Spread Footing engine result is invalid';
    return deepFreeze({
      ok: false,
      status: 'HOLD',
      errors: { engine: message },
      issues: [{ field: 'engine', code: 'ENGINE_FAIL_CLOSED', message, status: 'HOLD' }],
    });
  }
}
