import { buildSnapIndex, findSnap, validatePolyline, offsetPolyline, pitGeometry, placePitAtAnchor, fitPitToBounds, quickTrimExtendSegments } from './new-building-v2-cad-geometry.js?v=20260908-spacebar-repeat-v1';
...
export function mountCadWorkbench(options) {
  ...
  Object.assign(state, { editBoundary: null, editPreview: null, erasePreview: null, lastCommand: '' });
  const memberForms = {
  ...
  function command(value) {
    ...
    if (backgroundNativeContext() && ['B', 'COL'].includes(value)) {
      ...
      const tool = value === 'B' ? 'main-beam' : 'column';
      clearMode(); options.activateTool?.(tool);
      if (getActiveTool() === tool) { state.lastCommand = value; svg.focus?.(); }
      return;
    }
    if (['TR', 'EX'].includes(value)) {
      ...
      resetGesture(); startGesture(); state.mode = value; state.lastCommand = value;
  ...
  function executeText(text) {
    ...
    if (backgroundNativeContext()) {
      ...
      else if (parsed.kind === 'finish' && state.lastCommand) command(state.lastCommand);
  ...
  function onKey(event) {
    ...
    // A native selection is an SVG button; a removed selection can leave body
    // focused. Both still belong to plan command recall, unlike UI buttons.
    const repeatFromPlan = backgroundNativeContext() && event.key === ' ' && !event.ctrlKey && !event.metaKey && !event.shiftKey
      && !state.commandBuffer && state.lastCommand
      && (onPlan || (!toolButton && (event.target === doc.body || inToolControls)));
    ...
    if (toolButton && !prefixFromControls && !prefixFromPlanButton && !repeatFromPlan) return;
    if (!onPlan && !prefixFromControls && !repeatFromPlan) return;
    if (!keyboardAllowed()) {
    ...
    if (backgroundNativeContext()) {
      ...
      if (event.key === 'Enter' || event.key === ' ') {
        ...
        if (!state.commandBuffer && !editMode() && event.target !== svg && !repeatFromPlan) return;
        if (!state.commandBuffer && !state.lastCommand) return;
  ...
  function refresh() {
    ...
    // Toolbar commands participate in the same transient history as B/COL.
    // Select, Escape and model/level refreshes never replace the last command.
    if (backgroundEditing && state.tool && nextTool !== state.tool) {
      const nativeCommand = { 'main-beam': 'B', column: 'COL' }[nextTool];
      if (nativeCommand) state.lastCommand = nativeCommand;
    }
